...
import { ArrowLeft, MapPin, Clock, Phone, User, CheckCircle, Loader2, X, Navigation, ChevronDown, Package } from 'lucide-react'
...
export default function AntarPaketPage() {
  ...
  useEffect(() => {
    ...
    const savedPackage = localStorage.getItem('anterbae_delivery_package_detail')
  ...
  useEffect(() => {
    if (isLoaded) {
      localStorage.setItem('anterbae_delivery_package_detail', packageDetail)
    }
  ...
  const getWhatsAppLink = (ordNum: string) => {
    const waMessage = `*📦 PESANAN ANTAR PAKET (ANTERBAE)*\n\n` +
      ...
      `📦 Detail Paket: ${packageDetail || 'Barang/Paket'}\n\n` +
      `Terima kasih! Silakan proses pengiriman paket saya.`
  ...
  const handleSubmit = async () => {
    if (!pickup.trim() || !dropoff.trim() || !regName.trim() || !phone.trim() || !regAddress.trim() || !packageDetail.trim()) {
      alert('Silakan lengkapi seluruh data dan detail paket Anda terlebih dahulu.')
      return
    }
    ...
    const generatedOrderNumber = `ANTP-${Date.now().toString().slice(-6)}${rand}`
    ...
    try {
      ...
      fetch('/api/antar-jemput', {
        ...
        body: JSON.stringify({
          ...
          pickupTime: new Date().toISOString(), // default
          isRegistered: false,
          address: `${regAddress} (Detail Paket: ${packageDetail})`,
        }),
      }).catch(err => {
    ...
    <div className="min-h-screen bg-gray-50 pb-20">
      ...
      <div className="bg-white border-b border-gray-100 sticky top-0 z-50">
        <div className="flex items-center gap-3 px-4 py-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400 font-medium">Kirim paket cepat ke seluruh Banjarnegara</p>
          </div>
        </div>
      </div>
      ...
      <div className="px-4 py-4 space-y-4">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-4">
          ...
          <div className="relative">
            ...
            <input
              ...
              placeholder="Contoh: Dokumen, Makanan Kering, Baju, dll."
              className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-gray-200 bg-gray-50 text-sm font-bold text-gray-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent"
            />
          </div>
        </div>
        ...
        <button
          ...
          disabled={submitting || !pickup || !dropoff || !packageDetail}
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-6 w-full max-w-sm text-center">
            ...
            <p className="text-xs text-gray-400 mb-6 leading-relaxed">
              Detail pengiriman paket Anda sedang dialihkan ke WhatsApp. Driver/Admin akan segera memproses.
            </p>
